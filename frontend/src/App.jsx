...
function AppContent() {
        ...
        queryFn: async () => {
            ...
            if (!res.ok) {
                ...
                throw new Error(
                    errData.error ||
                        errData.message ||
                        `HTTP_ERROR_${res.status} // API menolak koneksi`
                );
            }
            return res.json();
        },
    ...
    const meta = qData?.total
        ? { total: qData.total, page: qData.page, totalPages: qData.totalPages }
        : null;
    ...
    const assembleMutation = useMutation({
        ...
        onSuccess: (data) => {
            playSound("success");
            showToast(
                `ASSEMBLY_SUCCESS: ${data.quantity} units assembled`,
                "success"
            );
            queryClient.invalidateQueries({ queryKey: ["items"] });
                ...
                (old) => {
                    ...
                    const processItem = (i) =>
                        i.id === id
                            ...
                            : i;
                    if (old.data)
                        return { ...old, data: old.data.map(processItem) };
        ...
        >
            ...
            (activePage === "dashboard" || activePage === "inventory") ? (
                ...
                <Routes>
                    ...
                    <Route
                        path="/production"
                        element={<ProductionBoard user={user} />}
                    />
                    {user?.role === "ADMIN" && (
                        <>
            ...
            {toast && (
                <motion.div
                    ...
                    exit={{ opacity: 0, y: 50 }}
                    className={`fixed bottom-6 left-1/2 z-50 px-6 py-3 rounded-xl border backdrop-blur-xl font-mono text-sm flex items-center gap-3 shadow-[0_0_30px_rgba(0,0,0,0.5)] ${
                        toast.type === "success"
