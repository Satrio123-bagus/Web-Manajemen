...
try {
    betterSqlite.exec(
        `ALTER TABLE items ADD COLUMN bab TEXT NOT NULL DEFAULT 'Uncategorized'`
    );
} catch (_) {}
try {
    betterSqlite.exec(
        `ALTER TABLE items ADD COLUMN sub_bab TEXT NOT NULL DEFAULT 'Uncategorized'`
    );
} catch (_) {}
try {
    betterSqlite.exec(
        `ALTER TABLE items ADD COLUMN location TEXT NOT NULL DEFAULT 'Belum Ditentukan'`
    );
} catch (_) {}
try {
    betterSqlite.exec(
        `ALTER TABLE items ADD COLUMN condition TEXT NOT NULL DEFAULT 'READY'`
    );
} catch (_) {}
try {
    betterSqlite.exec(
        `UPDATE items SET bab = category WHERE bab = 'Uncategorized' AND category IS NOT NULL AND category != ''`
    );
} catch (_) {}
try {
    betterSqlite.exec(
        `ALTER TABLE production_jobs ADD COLUMN supplier TEXT DEFAULT 'Campuran (Lama)'`
    );
} catch (_) {}
try {
    betterSqlite.exec(
...
const stmts = {
    ...
    getItemByName: {
        get: (name) =>
            db.select().from(items).where(eq(items.name, name)).get(),
    },
    insertItem: {
        run: (
            id,
            ...
            db
                ...
                .values({
                    ...
                    bab,
                    sub_bab,
                    location: location || "Belum Ditentukan",
    ...
    getProductionJobs: {
        all: () =>
            db
                ...
                .orderBy(desc(production_jobs.timestamp))
                .all(),
    },
    getProductionJobById: {
        get: (id) =>
            db
                .select()
                .from(production_jobs)
                .where(eq(production_jobs.id, id))
                .get(),
    },
    insertProductionJob: {
        run: (
    ...
    updateProductionJobStatus: {
        run: (id, status, catatan) =>
            db
                ...
                .set({ status, catatan: catatan || undefined })
                .where(eq(production_jobs.id, id))
                .run(),
    },
    deleteProductionJob: {
        run: (id) =>
            db.delete(production_jobs).where(eq(production_jobs.id, id)).run(),
    },
    upsertInventoryFromQC: {
        run: (job, totalLulus) => {
            ...
            if (existing) {
                db.update(items)
                    .set({ stock: existing.stock + totalLulus })
    ...
    getSupplyReports: {
        all: () =>
            db
                .select()
                .from(supply_reports)
                .orderBy(desc(supply_reports.timestamp))
                .all(),
    },
    insertSupplyReport: {
        run: (id, pekerja, laporan, status, timestamp) =>
            db
                .insert(supply_reports)
                .values({ id, pekerja, laporan, status, timestamp })
                .run(),
    },
    updateSupplyReportStatus: {
        run: (id, status) =>
            db
                .update(supply_reports)
                .set({ status })
                .where(eq(supply_reports.id, id))
                .run(),
    },
    insertTx: {
        run: (
        ...
        all: () =>
            db
                .select({
                    ...
                    total_sold: sql`SUM(${transactions.quantity})`.mapWith(
                        Number
                    ),
                    total_revenue: sql`SUM(${transactions.total})`.mapWith(
                        Number
                    ),
                })
                .from(transactions)
                .where(eq(transactions.type, "SALE"))
                .groupBy(transactions.item_name)
    ...
    getRevenueTotal: {
        get: () => {
            const res = db
                .select({
                    revenue:
                        sql`COALESCE(SUM(${transactions.total}), 0)`.mapWith(
                            Number
                        ),
                    sale_count: count(),
                })
    ...
    getDailyTrends: {
        all: () =>
            db
                ...
                .from(transactions)
                .where(eq(transactions.type, "SALE"))
                .groupBy(sql`strftime('%Y-%m-%d', ${transactions.timestamp})`)
                .orderBy(
                    desc(sql`strftime('%Y-%m-%d', ${transactions.timestamp})`)
                )
                .limit(7)
                .all(),
    },
    getSalesTrends: {
        ...
        all: (period) => {
            ...
            if (period === "daily") {
                ...
                orderSql = desc(
                    sql`strftime('%Y-%m-%d %H:00', ${transactions.timestamp})`
                );
                limit = 24;
                ...
                orderSql = desc(
                    sql`strftime('%Y-%m-%d', ${transactions.timestamp})`
                );
                limit = 7;
                ...
                orderSql = desc(
                    sql`strftime('%Y-%m', ${transactions.timestamp})`
                );
                limit = 12;
            } else {
                // monthly (default)
    ...
    getSalesStats: {
        get: (period) => {
            ...
            if (period === "daily")
                ...
                whereSql = sql`${transactions.timestamp} >= datetime('now', '-30 days', 'localtime')`; // monthly default
            ...
            const res = db
                .select({
                    ...
                    total_items:
                        sql`COALESCE(SUM(${transactions.quantity}), 0)`.mapWith(
                            Number
                        ),
                    tx_count: count(),
                })
    ...
    getDeadStock: {
        all: () => {
            ...
            return db
                ...
                .from(items)
                .where(
                    sql`${items.stock} > 0 AND NOT EXISTS (SELECT 1 FROM ${transactions} WHERE ${transactions.item_name} = ${items.name} AND ${transactions.timestamp} >= datetime('now', '-30 days', 'localtime'))`
                )
    ...
    insertConversation: {
        run: (session_id, role, content, timestamp) =>
            db
    ...
    insertReport: {
        run: (date, type, content, generatedBy, timestamp) => {
            const stmt = betterSqlite.prepare(
                "INSERT INTO reports (date, type, content, generated_by, timestamp) VALUES (?, ?, ?, ?, ?)"
            );
    ...
    getJobsForRollup: {
        get: () =>
            betterSqlite
                .prepare(
                    `SELECT * FROM production_jobs WHERE status IN ('SELESAI_JUAL', 'SELESAI_RAKIT', 'RUSAK')`
                )
                .all(),
    },
    insertAnalyticsRollup: {
        run: (
            id,
            bulan,
            supplier,
            ...
            betterSqlite
                .prepare(
                    `INSERT INTO supplier_analytics_rollup (id, bulan, supplier, tipe_remote, total_bagus, total_rusak, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)`
                )
                .run(
                    id,
                    bulan,
    ...
    deleteJobsForRollup: {
        run: () =>
            betterSqlite
                .prepare(
                    `DELETE FROM production_jobs WHERE status IN ('SELESAI_JUAL', 'SELESAI_RAKIT', 'RUSAK')`
                )
                .run(),
    },
    getAnalyticsRollup: {
        all: () =>
            betterSqlite
                .prepare(
                    `SELECT * FROM supplier_analytics_rollup ORDER BY bulan DESC, supplier ASC`
                )
                .all(),
    },
    getAllSupplyReports: {
        all: () =>
            betterSqlite
                .prepare("SELECT * FROM supply_reports ORDER BY timestamp DESC")
                .all(),
    },
    ...
    getAllPushSubs: {
        all: () =>
            betterSqlite.prepare("SELECT * FROM push_subscriptions").all(),
    },
    countPushSubs: {
        get: () =>
            betterSqlite
    ...
    getInventoryStats: {
        get: () => {
            const res = db
                .select({
                    totalItems: count(),
                    totalStock: sql`COALESCE(SUM(${items.stock}), 0)`.mapWith(
                        Number
                    ),
                    totalStockValue:
                        sql`COALESCE(SUM(${items.price} * ${items.stock}), 0)`.mapWith(
                            Number
                        ),
                    lowStockCount:
                        sql`COALESCE(SUM(CASE WHEN ${items.stock} < 2 THEN 1 ELSE 0 END), 0)`.mapWith(
                            Number
                        ),
                })
                .from(items)
...
console.log(
    `>> Loaded ${state.inventory.length} items from SQLite via Drizzle ORM`
);
...
try {
    ...
    const insertUser = betterSqlite.prepare(
    ...
    betterSqlite
        ...
        .run(newAdminHash, adminUser);

    // Update worker passwords to match .env (in case they changed .env later)
    betterSqlite
        .prepare("UPDATE users SET password_hash = ? WHERE username = ?")
        .run(bcrypt.hashSync(casingPass, 10), casingUser);
    betterSqlite
        .prepare("UPDATE users SET password_hash = ? WHERE username = ?")
        .run(bcrypt.hashSync(mesinPass, 10), mesinUser);

    console.log(
        ">> User roles & passwords enforced successfully based on .env."
    );
} catch (e) {
