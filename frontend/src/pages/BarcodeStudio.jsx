...
export default function BarcodeStudio() {
    ...
    const handleDownload = (locName, skuCount, totalStock) => {
        ...
        ctx.fillText(
            `${skuCount} Jenis  •  ${totalStock} Unit`,
            totalWidth / 2,
            infoY
        );
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm font-mono text-gray-500 mt-1">
                        Cetak atau unduh stiker Barcode untuk kotak penyimpanan
                        Anda.
                    </p>
                </div>
                <button
            ...
            <div
                className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 print-container"
                ref={qrRef}
            >
                {locations.map((loc) => {
                    const boxItems = locationGroups[loc];
                    const totalStock = boxItems.reduce(
                        (sum, i) => sum + i.stock,
                        0
                    );
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="w-full text-left space-y-2 mb-6 no-print">
                                <div className="flex justify-between items-center text-xs font-mono text-gray-400">
                                    <span className="flex items-center gap-1">
                                        <Layers className="w-3 h-3" /> SKU
                                        Count:
                                    </span>
                                    <span className="text-white font-bold">
                                        {boxItems.length} Jenis
                                    </span>
                                </div>
                                <div className="flex justify-between items-center text-xs font-mono text-gray-400">
                                    <span className="flex items-center gap-1">
                                        <Box className="w-3 h-3" /> Total Fisik:
                                    </span>
                                    <span className="text-white font-bold">
                                        {totalStock} Unit
                                    </span>
                            ...
                            <div className="hidden print:block w-full text-left mt-4 text-black text-sm font-mono">
                                <p>
                                    SKU Count: {boxItems.length} | Total Items:{" "}
                                    {totalStock}
                                </p>
                            </div>
