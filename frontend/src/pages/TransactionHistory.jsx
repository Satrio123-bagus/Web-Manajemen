...
import {
    ...
    TrendingUp, Package, RefreshCw, Trash2, Edit3, Plus, Search
} from 'lucide-react';
import api from '../api';

/* ── Helpers ── */
const TYPE_CONFIG = {
    SALE:    { label: 'Penjualan',  color: 'text-cyan-400',    bg: 'bg-cyan-400/10',    border: 'border-cyan-400/30',    dot: 'bg-cyan-400' },
    RESTOCK: { label: 'Restock',    color: 'text-violet-400',  bg: 'bg-violet-400/10',  border: 'border-violet-400/30',  dot: 'bg-violet-400' },
    CREATE:  { label: 'Item Baru',  color: 'text-emerald-400', bg: 'bg-emerald-400/10', border: 'border-emerald-400/30', dot: 'bg-emerald-400' },
    UPDATE:  { label: 'Edit',       color: 'text-amber-400',   bg: 'bg-amber-400/10',   border: 'border-amber-400/30',   dot: 'bg-amber-400' },
    DELETE:  { label: 'Hapus',      color: 'text-red-400',     bg: 'bg-red-400/10',     border: 'border-red-400/30',     dot: 'bg-red-400' },
    ADD:     { label: 'Tambah',     color: 'text-emerald-400', bg: 'bg-emerald-400/10', border: 'border-emerald-400/30', dot: 'bg-emerald-400' },
};
...
function getTypeConfig(type) {
    return TYPE_CONFIG[type] || { label: type, color: 'text-gray-400', bg: 'bg-gray-400/10', border: 'border-gray-400/30', dot: 'bg-gray-400' };
}
...
const ALL_TYPES = ['ALL', 'SALE', 'RESTOCK', 'CREATE', 'UPDATE', 'DELETE', 'ADD'];
const PAGE_SIZE = 50;
        ...
        <div className="min-h-screen bg-[#080c14] text-white font-mono p-4 md:p-6 max-w-[1400px] mx-auto">
            ...
            >
                ...
                <div>
                    ...
                    <p className="text-[10px] text-gray-600">{allTx.length} total records • {filtered.length} ditampilkan</p>
                </div>
                <button
                    ...
                    title="Refresh"
                >
                    <RefreshCw className={`w-4 h-4 text-gray-400 ${loading ? 'animate-spin text-[#00f3ff]' : ''}`} />
                </button>
            </motion.div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                {[
                    { label: 'Total Transaksi', value: allTx.length, color: '#00f3ff' },
                    { label: 'Total Penjualan', value: salesCount, color: '#bc13fe' },
                    { label: 'Total Restock', value: restockCount, color: '#a78bfa' },
                    { label: 'Total Revenue', value: formatRp(salesTotal), color: '#34d399', isText: true },
                ].map((s, i) => (
                    <motion.div
                        key={s.label}
                    ...
                    >
                        <p className="text-[9px] text-gray-500 uppercase tracking-widest mb-1">{s.label}</p>
            ...
            <div className="flex flex-col md:flex-row gap-3 mb-5">
                ...
                <div className="flex gap-1.5 flex-wrap">
                    ...
                    {ALL_TYPES.map(t => {
                        const cfg = t === 'ALL' ? { color: 'text-gray-300', bg: 'bg-white/10', border: 'border-white/20' } : getTypeConfig(t);
            ...
            <div className="bg-white/[0.02] rounded-xl border border-white/[0.05] overflow-hidden">
                ...
                <div className="divide-y divide-white/[0.03]">
                    {loading ? (
                        ...
                        <div className="text-center py-16 text-gray-600 text-xs">Tidak ada transaksi yang cocok.</div>
                    ) : (
                        <AnimatePresence mode="popLayout">
                            {paginated.map((tx, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Type Badge */}
                                        <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-bold border ${cfg.bg} ${cfg.color} ${cfg.border} shrink-0`}>
                                            {TYPE_ICONS[tx.type]}
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-xs text-white font-bold truncate">{tx.item_name || '—'}</p>
                                            <p className="text-[9px] text-gray-600 truncate">{tx.category || tx.transaction_id}</p>
                                        </div>

                                        {/* Qty */}
                                        <div className="hidden md:block text-right">
                                            <p className="text-xs text-gray-300">{tx.quantity > 0 ? `×${tx.quantity}` : '—'}</p>
                                        </div>

                                        {/* Total */}
                                        <div className="hidden md:block text-right">
                                            <p className={`text-xs font-bold ${tx.type === 'SALE' ? 'text-cyan-400' : 'text-gray-600'}`}>
                                                {tx.total > 0 ? formatRp(tx.total) : '—'}
                                            </p>
                                        </div>

                                        {/* Timestamp */}
                                        <div className="text-right shrink-0">
                                            <p className="text-[10px] text-gray-400">{formatDate(tx.timestamp)}</p>
                                            <p className="text-[9px] text-gray-600">{formatTime(tx.timestamp)}</p>
                                        </div>
                                    </motion.div>
                                );
                            })}
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4 text-xs text-gray-600">
                    <span>{filtered.length} records • halaman {currentPage}/{totalPages}</span>
