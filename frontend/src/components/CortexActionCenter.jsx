...
import { BrainCircuit, AlertTriangle, Package, ShoppingCart } from 'lucide-react';
import api from '../api';
...
export default function CortexActionCenter({ onAssemble }) {
    ...
    const renderActionItem = (item) => {
        ...
        const isAssemblable = item.bab === 'WIP' || item.category === 'WIP' || item.bab === 'SPARE_PART' || item.category === 'SPARE_PART' || item.name.toLowerCase().includes('casing') || item.name.toLowerCase().includes('bahan');

        return (
            <div key={item.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 rounded-xl transition-colors group">
                <div className="flex items-start gap-3 mb-3 sm:mb-0">
                    <div className={`p-2 rounded-lg ${isOutOfStock ? 'bg-red-500/10 text-red-500' : 'bg-amber-400/10 text-amber-400'}`}>
                    ...
                    <div>
                        ...
                        <p className="font-mono text-[10px] text-gray-500 mt-1">
                            {isOutOfStock ? (
                                <span className="text-red-400">OUT OF STOCK (0 Unit)</span>
                            ) : (
                                <span className="text-amber-400">CRITICAL STOCK ({item.stock} Unit)</span>
                            )}
                            {" | "}
                            <span className="uppercase tracking-widest">{item.bab || item.category}</span>
                        </p>
                    </div>
                </div>
                ...
                <div>
                    {isAssemblable ? (
                        ...
                        <motion.button
                            ...
                            onClick={() => window.alert('Catatan belanja berhasil disalin. Silakan pesan ke supplier.')}
                        >
                ...
                <div className="flex-1 p-5 space-y-3 overflow-y-auto relative z-10 max-h-[400px]">
                    {loading ? (
                        ...
                        <div className="py-12 text-center font-mono text-gray-500">
                            ...
                            <p className="text-sm font-bold text-emerald-400">STATUS AMAN (OPTIMAL)</p>
                            <p className="text-xs mt-1">Tidak ada aksi darurat yang diperlukan saat ini.</p>
                        </div>
                    )}
                </div>
            </div>
