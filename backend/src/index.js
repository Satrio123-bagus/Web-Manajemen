    ...
    cors({
        origin: (origin, callback) => {
            ...
            if (
                ...
                return callback(
                    new Error(`CORS: Origin HTTP plaintext ditolak: ${origin}`),
                    false
                );
            }
...
const limiter = rateLimit({
    ...
    validate: false,
    message: {
        error: "RATE_LIMIT_EXCEEDED // Too many requests, try again later.",
    },
});
app.use("/api/", limiter);
...
app.use((req, res, next) => {
    if (
        req.path.startsWith("/api/terminal/vision") ||
...
const loginLimiter = rateLimit({
    ...
    validate: false,
    message: {
        error: "LOGIN_RATE_LIMIT // Terlalu banyak percobaan login yang gagal. Coba lagi dalam 15 menit.",
    },
});
...
const aiLimiter = rateLimit({
    ...
    validate: false,
    message: {
        error: "RATE_LIMIT_EXCEEDED // AI Engine Cooling Down. Try again in a minute.",
    },
});
...
const server = app.listen(PORT, () => {
    ...
    hermes
        ...
        .then((available) => {
            if (available) {
                console.log(
                    `  >> HERMES AGENT: Online ✓ (${hermes.MODEL_NAME})`
                );
            } else {
                console.log(
                    `  >> HERMES AGENT: Offline (model akan di-pull saat pertama dipakai)`
                );
            }
        })
        .catch(() => {
        ...
        async () => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    const preview = result.report
                        ? result.report.slice(0, 120) + "..."
                        : "";
        ...
        async () => {
            try {
                ...
                if (lowStock.length > 0) {
                    console.log(
                        `[CRON:STOCK] ${lowStock.length} item stok kritis → kirim push alert`
                    );
        ...
        async () => {
            ...
            try {
                ...
                if (result.anomalyCount > 0) {
                    console.log(
                        `[CRON:ANOMALY] ✓ ${result.anomalyCount} anomali ditemukan dan dilaporkan.`
                    );
                } else {
                    console.log(
                        "[CRON:ANOMALY] ✓ Tidak ada anomali signifikan, sistem normal."
                    );
                }
            } catch (err) {
        ...
        () => {
            ...
            try {
                ...
                if (!fs.existsSync(DB_SOURCE)) {
                    console.warn("[CRON:BACKUP] DB tidak ditemukan.");
                    return;
                }

                fs.copyFileSync(DB_SOURCE, backupFile);
                console.log(
                    `[CRON:BACKUP] ✓ Database di-backup ke: ${backupFile}`
                );
                ...
                const files = fs
                    ...
                    .filter(
                        (f) => f.startsWith("inventory_") && f.endsWith(".db")
                    )
                    .map((f) => ({
                        name: f,
                        time: fs
                            .statSync(path.join(BACKUP_DIR, f))
                            .mtime.getTime(),
                    }))
                ...
                if (files.length > MAX_BACKUPS) {
                    files.slice(MAX_BACKUPS).forEach((f) => {
                        ...
                        console.log(
                            `[CRON:BACKUP] 🗑️ Hapus backup lama: ${f.name}`
                        );
                    });
                }
                console.log(
                    `[CRON:BACKUP] Total backup: ${Math.min(files.length, MAX_BACKUPS)}/${MAX_BACKUPS}`
                );
            } catch (err) {
