...
export default function Login({ onLogin }) {
    ...
    return (
        <div className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
            ...
            {/* Tombol Low Graphic Darurat untuk Windows 7 */}
            <div className="absolute top-4 right-4 z-50">
                <button
                    onClick={() => toggleSetting("lowGraphics")}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border font-mono text-xs transition-colors ${
                        settings.lowGraphics
                            ? "bg-[var(--color-neon-cyan)]/20 text-[var(--color-neon-cyan)] border-[var(--color-neon-cyan)]/50 shadow-[0_0_10px_rgba(0,243,255,0.2)]"
                            : "bg-black/50 text-gray-500 border-white/10 hover:text-white"
                    }`}
                >
                    <div
                        className={`w-2 h-2 rounded-full ${settings.lowGraphics ? "bg-[var(--color-neon-cyan)] animate-pulse" : "bg-gray-600"}`}
                    />
                    {settings.lowGraphics ? "WIN7 MODE: ON" : "WIN7 MODE: OFF"}
                </button>
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {error && (
                        <motion.div
                            ...
                            className="p-3 bg-red-500/10 border border-red-500/30 rounded-xl flex items-center gap-2 text-red-400 text-xs font-mono"
                        >
