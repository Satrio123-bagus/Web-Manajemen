...
function StatCard({ icon: Icon, label, value, suffix, accent, delay = 0 }) {
    return (
        ...
        >
            ...
            <div
                className={`absolute -inset-px rounded-2xl bg-gradient-to-br ${accent} opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-sm`}
            />
            <div
                className={`absolute -inset-px rounded-2xl bg-gradient-to-br ${accent} opacity-30 group-hover:opacity-60 transition-opacity duration-500`}
            />
            ...
            <div className="relative bg-[rgba(8,8,12,0.85)] backdrop-blur-xl rounded-2xl p-6 border border-white/5 group-hover:border-transparent transition-all duration-300 overflow-hidden">
                ...
                <div className="relative z-10 flex items-start justify-between">
                    <div>
                        <p className="text-[10px] font-mono tracking-[0.2em] text-gray-500 uppercase mb-3">
                            {label}
                        </p>
                ...
                {/* Bottom accent line */}
                <div
                    className={`absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r ${accent} opacity-40 group-hover:opacity-100 transition-opacity duration-300`}
                />
            </div>
        </motion.div>
    );
...
export default function Dashboard({
    ...
    const wipItems = useMemo(
        () => items.filter((i) => i.condition === "WIP"),
        [items]
    );
    ...
    const filtered = useMemo(() => {
        ...
        if (filterKategori !== "Semua") {
            result = result.filter((i) => {
                ...
                if (filterKategori === "Casing") return name.includes("casing");
                if (filterKategori === "Mesin") return name.includes("mesin");
                if (filterKategori === "Tutup") return name.includes("tutup");
                if (filterKategori === "Layar") return name.includes("layar");
                if (filterKategori === "Lainnya")
                    return (
        ...
        return result;
    }, [items, sortKey, sortDir, filterKategori]);
        ...
        <div className="max-w-[1400px] mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard
                    ...
                    suffix="Rp"
                    accent="from-[var(--color-neon-cyan)] to-blue-500"
                    delay={0}
                />
                <StatCard
                    icon={Package}
                    label="System Load"
                    value={meta ? meta.total : stats.totalItems}
                    suffix="ITEMS"
                    accent="from-[var(--color-neon-purple)] to-pink-500"
                    delay={0.1}
                />
                <StatCard
                    icon={AlertTriangle}
                    ...
                    >
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-start gap-4">
                            ...
                            <div className="flex-1 w-full">
                                <h3 className="text-lg font-bold text-amber-400 mb-1 font-mono tracking-wide">
                                    BENGKEL / QUALITY CONTROL
                                </h3>
                                <p className="text-sm text-amber-400/70 mb-4 max-w-2xl">
                                    Terdapat{" "}
                                    <span className="font-bold text-white">
                                        {wipItems.length} barang
                                    </span>{" "}
                                    yang belum selesai diproses (WIP).
                                    Barang-barang ini otomatis{" "}
                                    <strong className="text-red-400">
                                        dikunci dari penjualan
                                    </strong>{" "}
                                    oleh sistem Cortex sampai diperbaiki/selesai
                                    dirakit.
                                </p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                                    {wipItems.map((item) => (
                                        <div
                                            key={item.id}
                                            className="bg-black/60 border border-amber-500/20 p-3 rounded-xl flex justify-between items-center group cursor-pointer hover:border-amber-400/60 hover:bg-amber-900/20 transition-all shadow-[inset_0_0_15px_rgba(251,191,36,0.05)]"
                                            onClick={() => onEdit(item)}
                                        >
                                            <div className="min-w-0 pr-2">
                                                <p className="font-bold text-amber-300 text-sm truncate group-hover:text-amber-200">
                                                    {item.name}
                                                </p>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
                        <div>
                            ...
                            <p className="text-xs font-mono text-gray-600 mt-1">
                                {meta
                                    ? `Showing page ${meta.page} of ${meta.totalPages || 1} (${meta.total} records)`
                                    : `${filtered.length} of ${items.length} records displayed`}
                            </p>
                        </div>
                        <div className="flex gap-3 items-center">
                            {/* Kategori Dropdown */}
                            <div className="relative flex items-center bg-[#0a0a0c] border border-white/10 rounded-lg h-9 px-3 hover:border-[var(--color-neon-purple)]/50 transition-colors">
                                <select
                                    value={filterKategori}
                                    onChange={(e) =>
                                        setFilterKategori(e.target.value)
                                    }
                                    className="bg-transparent border-none outline-none text-xs text-white font-mono cursor-pointer appearance-none pr-4"
                                >
                                    <option value="Semua">
                                        Semua Kategori
                                    </option>
                                    ...
                                    <option value="Tutup">Tutup Baterai</option>
                                                ...
                                                >
                                                    ...
                                                    <td className="px-5 py-4">
                                                        <span className="font-bold text-white group-hover/row:text-[var(--color-neon-cyan)] transition-colors flex items-center gap-2">
                                                            ...
                                                            {item.condition ===
                                                                "WIP" && (
                                                                <span className="px-1.5 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/30 text-[9px] rounded font-mono font-bold tracking-widest uppercase shadow-[0_0_10px_rgba(251,191,36,0.2)]">
                                                                    WIP
                                                                </span>
                                                            )}
                                                        </span>
                                                    ...
                                                    <td className="px-5 py-4">
                                                        <span
                                                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-mono tracking-widest uppercase border ${rarity.text} ${rarity.bg} ${rarity.border}`}
                                                        >
                                                            <span
                                                                className={`w-1.5 h-1.5 rounded-full ${rarity.dot}`}
                                                            />
                                                            {item.rarity ===
                                                            "LANGKA"
                                                                ? "Remote Langka"
                                                    ...
                                                    <td className="px-5 py-4">
                                                        <span className="font-mono text-white group-hover/row:text-[var(--color-neon-cyan)] transition-colors">
                                                            ...
                                                            <span className="text-[10px] text-gray-400 ml-1 font-normal uppercase tracking-tighter">
                                                                Rp
                                                            </span>
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="px-5 py-4">
                                                        <div className="flex items-center gap-3">
                                                            <span
                                                                className={`font-mono font-bold tabular-nums ${item.stock < 2 ? "text-amber-400" : "text-white"}`}
                                                            >
                                                                {item.stock}
                                                            </span>
                                                            {/* Mini bar */}
                                                    ...
                                                    <td className="px-5 py-4">
                                                        <span
                                                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-mono tracking-widest border ${status.color} ${status.bg} ${status.border}`}
                                                        >
                                                            <span
                                                                className={`w-1.5 h-1.5 rounded-full ${status.dot} ${item.stock < 2 ? "animate-pulse" : ""}`}
                                                            />
                                                            {status.label}
                                                        </span>
                                                    </td>
                                                        ...
                                                        <div className="flex gap-1 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                                            ...
                                                            {item.price > 0 && (
                                                                <motion.button
                                                                    ...
                                                                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-mono font-bold flex items-center gap-1 transition-all border ${
                                                                        item.stock >
                                                                            0 &&
                                                                        item.condition !==
                                                                            "WIP"
                                                                    ...
                                                                    title={
                                                                        item.condition ===
                                                                        "WIP"
                                                                            ? "Terkunci: Masih WIP (Belum Selesai)"
                                                                            : item.stock >
                                                                                0
                                                                              ? `Sell 1x ${item.name}`
                                                                              : "Out of stock"
                        ...
                        <div className="px-5 py-3 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div className="flex items-center gap-4">
                                <p className="text-[10px] font-mono text-gray-600">
                                    TOTAL_ASSETS:{" "}
                                    <span className="text-[var(--color-neon-cyan)]">
                                        {stats.totalAssets.toLocaleString()} Rp
                                    </span>
                                </p>
                            ...
                            {meta && meta.totalPages > 1 && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] text-[var(--color-neon-cyan)] font-mono tracking-widest px-2">
                                        [{String(meta.page).padStart(2, "0")} /{" "}
                                        {String(meta.totalPages).padStart(
                                            2,
                                            "0"
                                        )}
                                        ]
...
export function InventoryModal({ isOpen, onClose, onSave, initialData }) {
    ...
    const formData = isOpen
        ? initialData
            ? {
                  ...initialData,
                  price: String(initialData.price),
                  stock: String(initialData.stock),
                  bab: initialData.bab || initialData.category || "",
                  sub_bab: initialData.sub_bab || "",
                    ...
                    >
                        ...
                        >
                            ...
                            <FieldInput
                                ...
                                placeholder="Kotak A, Rak B, Laci C..."
                            />
                            <div className="grid grid-cols-2 gap-4">
                                <FieldInput
                                    label="UNIT_COST"
                            ...
                            <div className="relative">
                                <label className="block text-[10px] uppercase font-mono tracking-widest text-[var(--color-neon-cyan)] mb-1 opacity-70">
                                    TIPE_REMOTE
                                </label>
                                <select
                                    value={form.rarity}
...
function FieldInput({ label, type = "text", value, onChange, placeholder }) {
    return (
        <div className="relative group">
            <label className="block text-[10px] uppercase font-mono tracking-widest text-[var(--color-neon-cyan)] mb-1 opacity-70 group-focus-within:opacity-100 transition-opacity">
                {label}
            </label>
            <input
                type={type}
...
export function AssembleModal({
        ...
        if (isOpen && sourceItem) {
            ...
            if (allItems) {
                ...
                if (baseNameMatch && baseNameMatch[1]) {
                    ...
                    const targetMatch = allItems.find(
                        (i) =>
                            i.name.toLowerCase() === baseName &&
                            i.id !== sourceItem.id
                    );
                    if (targetMatch) {
    ...
    const validTargets = allItems
        ? allItems.filter((i) => !materialIds.includes(i.id))
        : [];
    const availableMaterials = allItems
                        ...
                        >
                            ...
                            <div className="p-4 rounded-xl border border-[var(--color-neon-cyan)]/20 bg-[var(--color-neon-cyan)]/5">
                                <h3 className="text-xs font-mono text-[var(--color-neon-cyan)] mb-3 flex items-center gap-2">
                                    <Package className="w-4 h-4" /> 1. HASIL
                                    RAKITAN (TARGET)
                                </h3>
                            ...
                            <div className="p-4 rounded-xl border border-[var(--color-neon-purple)]/20 bg-[var(--color-neon-purple)]/5">
                                <div className="flex justify-between items-center mb-3">
                                    <h3 className="text-xs font-mono text-[var(--color-neon-purple)] flex items-center gap-2">
                                        <Plus className="w-4 h-4" /> 2. BAHAN
                                        BAKU (MATERIALS)
                                    </h3>
                                    <button
                                        type="button"
